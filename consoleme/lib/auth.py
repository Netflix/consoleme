...
def can_admin_all(user: str, user_groups: List[str]):
    ...
    if application_admin:
        if user == application_admin or application_admin in user_groups:
            return True
    if is_in_group(user, user_groups, [*config.get("groups.can_admin", []), *config.get("dynamic_config.groups.can_admin", [])]):
        return True
    return False
...
def can_create_roles(
    ...
    if can_admin_all(user, user_groups):
        return True
    if is_in_group(user, user_groups, [*config.get("groups.can_create_roles", []), *config.get("dynamic_config.groups.can_create_roles", [])]):
        return True
    return False
...
def can_admin_policies(user: str, user_groups: List[str]) -> bool:
    if can_admin_all(user, user_groups):
        return True
    if is_in_group(user, user_groups, [*config.get("groups.can_admin_policies", []), *config.get("dynamic_config.groups.can_admin_policies", [])]):
        return True
    return False


def can_delete_roles_app(app_name):
    if app_name in [*config.get("groups.can_delete_roles_apps", []), *config.get("dynamic_config.groups.can_delete_roles_apps", [])]:
        return True
    return False
...
def can_delete_roles(
    ...
    if can_admin_all(user, user_groups):
        return True
    if is_in_group(user, user_groups, [*config.get("groups.can_delete_roles", []), *config.get("dynamic_config.groups.can_delete_roles", [])]):
        return True
    return False
...
def can_edit_dynamic_config(
    ...
    if can_admin_all(user, user_groups):
        return True
    if is_in_group(user, user_groups, [*config.get("groups.can_edit_config", []), *config.get("dynamic_config.groups.can_edit_config", [])]):
        return True
    return False
...
def can_modify_members(
    ...
    if is_in_group(user, user_groups, [*config.get("groups.can_modify_members", []), *config.get("dynamic_config.groups.can_modify_members", [])]):
        return True
    return False
...
def can_edit_sensitive_attributes(
    ...
    if can_admin_all(user, user_groups):
        return True
    if is_in_group(
        user, user_groups, [*config.get("groups.can_edit_sensitive_attributes", []), *config.get("dynamic_config.groups.can_edit_sensitive_attributes", [])]
    ):
        return True
    return False
...
def is_sensitive_attr(attribute):
    for attr in [*config.get("groups.attributes.boolean", []), *config.get("dynamic_config.groups.attributes.boolean", [])]:
        if attr.get("name") == attribute:
            return attr.get("sensitive", False)

    for attr in [*config.get("groups.attributes.list", []), *config.get("dynamic_config.groups.attributes.list", [])]:
        if attr.get("name") == attribute:
            return attr.get("sensitive", False)
    return False
