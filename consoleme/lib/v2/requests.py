        ...
        if policy_change.change_type == "inline_policy":
            ...
            for resource in policy_change.resources:
                ...
                if (
                    ...
                    # resource_account_id != role_account_id and # TODO: remove this comment
                    ...
                    for statement in policy_change.policy.policy_document.get(
                        ...
                        if resource.arn in statement.get("Resource"):
                            # check if action is sts:AssumeRole and the sole action in the statement (sanity check)
                            actions = get_actions_for_resource(resource.arn, statement)
                            if len(actions) == 1 and actions[0] == "sts:AssumeRole":
                                # Cross account sts assume role policy
                                auto_generated_resource_policy_changes.append(
                                    ResourcePolicyChangeModel(
                                        arn=resource.arn,
                                        policy=PolicyModel(
...
async def populate_cross_account_resource_policy_for_change(
    ...
    if (
        ...
        if change.change_type == "resource_policy":
            ...
            # sts assume role policy change - fixed action
            resource_arns = []
...
async def maybe_approve_reject_request(
    ...
    for change in extended_request.changes.changes:
        ...
        if change.status == Status.not_applied:
            ...
            if change.change_type == "resource_policy" and change.supported is False:
                continue
