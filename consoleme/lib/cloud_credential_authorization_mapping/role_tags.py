...
from consoleme.lib.cloud_credential_authorization_mapping.models import CredentialAuthzMappingGenerator, user_or_group, \
    RoleAuthorizations
...
class RoleTagAuthorizationMappingGenerator(CredentialAuthzMappingGenerator):
    ...
    async def generate_credential_authorization_mapping(
            self, authorization_mapping: Dict[user_or_group, RoleAuthorizations]) -> Dict[
        ...
        for arn, role_entry_j in all_roles.items():
            ...
            if (required_trust_policy_entity
                and required_trust_policy_entity.lower() not in json.dumps(policy["AssumeRolePolicyDocument"],
                                                                           escape_forward_slashes=False).lower()):
                continue
