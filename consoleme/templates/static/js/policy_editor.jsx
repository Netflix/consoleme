...
export async function AddManagedPolicy(policy_name, policy_arn, role_arn) {
  ...
  let result = await Swal.fire({
    title: "Are you sure?",
    text:
      "Are you sure you want to add " + policy_arn + " to " + role_arn + "?",
    type: "warning",
...
export async function editPolicy(
  ...
  if (typeof policy_name == "undefined" || policy_name === "") {
    Swal.fire("Policy Error", "Policy name must not be empty", "error");
    return false;
  }

  let lint_errors = editor_value.getSession().getAnnotations();
  if (lint_errors.length > 0) {
    Swal.fire("Lint Error", JSON.stringify(lint_errors), "error");
    return false;
  }
  ...
  let arr = [
    {
      type: policy_type,
      name: policy_name,
      value: eval(editor_value).getValue(),
      is_new: is_new,
    },
  ];
  let json = JSON.stringify(arr);
  ...
  if (justification) {
    ...
    if (!admin_auto_approve) {
      ...
      await handleResponse(
        res,
        redirect_uri,
        "Success! Redirecting to the request."
      );
    } else {
      // admin submitted self-approved request
      let requestLink =
        '<a href="/policies/request/' +
        res.request_id +
        '" target="_blank"> here </a>';
      let htmlMessage =
        "Your policy request was successfully self-approved and applied. Please click " +
        requestLink +
...
export async function deletePolicy(policy_type, policy_name) {
  let r = confirm(
    "Are you sure you want to delete " + policy_name + " from this role?"
  );
  if (r !== true) {
    return false;
  }
...
export async function addTagField() {
  ...
  let new_tag =
    ...
    '                        <input type="text" placeholder="Name" name="newtag_' +
    name +
    '" value="nflxtag-" id="' +
    name +
    '" value="nflxtag-">\n' +
    ...
    '                        <input type="text" placeholder="Value" name="newtagvalue_' +
    name +
    '" id="' +
    name +
    '_value">\n' +
    "                    </div>\n" +
    '<div class="field"><button class="ui red button" style="margin: 23px;width: 150px;" type="button" onClick="$(\'#' +
    name +
    "div').remove();\">" +
...
export async function changeNewPolicyTemplate(templateDD) {
  ...
  neweditor.setValue(
    JSON.stringify(JSON.parse(neweditor.getSession().getValue()), null, 4)
  );
}
...
export function renderIAMSelfServiceWizard() {
  ...
  ReactDOM.render(
    <Provider store={store}>
      <SelfServiceForm
        ref={(SelfServiceForm) => {
          window.SelfServiceForm = SelfServiceForm;
        }}
        isHidden={false}
        arn={arn}
        account_id={account_id}
...
export async function deleteRole(account_id, role_name) {
  ...
  if (result.value) {
    ...
    await handleResponse(
      resJson,
      "/policies",
      "Successfully deleted role! Redirecting to ConsoleMe's policies page",
      5000
    );
  }
}
...
export async function applyResourcePolicy(
  ...
  let url =
    ...
    resource_type +
    "/" +
    (region === "" ? resource_name : region + "/" + resource_name);
  let arr = [
    {
      type: "ResourcePolicy",
      name: "Resource Policy",
      value: editor_value.getValue(),
      is_new: is_new,
    },
  ];
  let json = JSON.stringify(arr);
