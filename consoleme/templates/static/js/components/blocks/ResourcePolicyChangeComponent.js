...
class ResourcePolicyChangeComponent extends Component {
  ...
  render() {
    ...
    const applyChangesButton = config.can_approve_reject && change.status === 'not_applied' && change.supported
      ? (
        <Grid.Column>
          <Button
    ...
    const updateChangesButton = config.can_update_cancel && change.status === 'not_applied' && change.supported
      ? (
        <Grid.Column>
          <Button
    ...
    const changesAlreadyAppliedContent = (change.status === 'applied') && change.supported
      ? (
        <Grid.Column>
          <Message info>
      ...
      ? (
        <Grid.Column>
          <Message warning>
            ...
            <p>This resource type is currently not supported for automatic
              application of resource policies. A best effort proposed resource
              policy is shown above. Please review carefully before applying.
            </p>
          </Message>
        </Grid.Column>
      )
