...
class RequestHandler(BaseAPIV2Handler):
    ...
    async def post(self):
        ...
        if extended_request.request_status == RequestStatus.approved:
            ...
            log.debug(log_data)
        extended_request_result = await populate_cross_account_resource_policies(
            extended_request, self.user
        )
        extended_request = extended_request_result["extended_request"]
        has_autogenerated_policies = False
        for change in extended_request.changes.changes:
            if change.autogenerated:
                has_autogenerated_policies = True
        await aws.send_communications_new_policy_request(
            extended_request,
            admin_approved,
            approval_probe_approved,
            has_autogenerated_policies,
        )
