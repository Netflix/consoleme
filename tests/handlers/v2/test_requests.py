...
class TestRoleRequestsHandler(AsyncHTTPTestCase):
    def get_app(self):
        from consoleme.routes import make_app
    ...
    def test_get(self):
        headers = {"Authorization": "Bearer foo"}
        response = self.fetch(
            "/api/v2/roles/012345678901/fake_account_admin/requests",
            method="GET",
            headers=headers,
        )
        expected = {
    ...
    def test_post(self):
        ...
        expected = {
            "status": 501,
            "title": "Not Implemented",
            "message": "Create request for role",
        }
        self.assertEqual(response.code, 501)
        self.assertDictEqual(json.loads(response.body), expected)
