  ...
  switch (action.type) {
    ...
    case "CREATE_TAG":
      return {
        ...state,
        isNewTag: false,
        tags: [{ Key: action.created.key, Value: action.created.value, New: true }, ...state.tags],
    ...
    case "DELETE_TAG":
      ...
      const createdTags = newChanges.reduce((created, change) => {
        if (change.tag_action === "create" && change.key === action.deleted.key) {
      ...
      return {
        ...
        tagChanges: newChanges.filter(change => !createdTags.includes(change.key))
      };
    case "UPDATE_TAG":
      const matched = state.tagChanges.filter(change => change.original_key === action.changed.original_key);
