...
const Permissions = {
  "S3": {
    ...
    "get": ["s3:GetObject", "s3:GetObjectTagging", "s3:GetObjectVersion", "s3:GetObjectVersionTagging", "s3:GetObjectAcl", "s3:GetObjectVersionAcl"],
    "put": ["s3:PutObject", "s3:PutObjectTagging", "s3:PutObjectVersionTagging", "s3:ListMultipartUploadParts*", "s3:AbortMultipartUpload", "s3:RestoreObject"],
    "delete": ["s3:DeleteObject", "s3:DeleteObjectTagging", "s3:DeleteObjectVersion", "s3:DeleteObjectVersionTagging"]
  },
  "SQS": {
  ...
  "EC2": {
    "volmount": ["ec2:attachvolume", "ec2:createvolume", "ec2:describelicenses", "ec2:describevolumes", "ec2:detachvolume", "ec2:reportinstancestatus", "ec2:resetsnapshotattribute"]
  },
  "RDS": {
...
function generatePolicy(choices, policy_sid, state) {
  ...
  if (choices.choose === "S3") {
    ...
    if (choices.passrole === true) {
      resources.push("arn:aws:iam::" + account_id + ":role/rds-monitoring-role")
    }
...
let SelfServiceFormPage3 = props => {
  ...
  return (
    <Form onSubmit={handleSubmit}>
      <Form.Field>
        <div>Please modify the policy if needed, then click the "Submit" button when done.</div>
        <div>Reference the <a href="https://awspolicygen.s3.amazonaws.com/policygen.html" target="_blank">AWS
          Policy Generator</a> if you need a more complex set of permissions.
        </div>
        <br/>
        {choices.wizard_policy_editor}
      </Form.Field>
      <Form.Field>
        <input type="hidden" id="policy_sid" name="policy_sid" value={policy_sid}></input>
      </Form.Field>
      <button type="button" className="ui positive button previous" onClick={previousPage} style={{margin: "10px"}}>
        Previous
      </button>
      <button type="submit" className="ui positive button submit" disabled={pristine || submitting}
              style={{margin: "10px"}}>
        Submit
      </button>
    </Form>
