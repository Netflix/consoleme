...
export class CustomSearchField extends Component {
  constructor(props) {
    super(props);
    this.searchChange = _.debounce(this.handleSearchChange, 500, {leading: true});
    this.state = {
      isLoading: false,
  ...
  fetchResults(value) {
    let url = "/policies/typeahead?resource=" + this.state.resource_type + "&search=" + value
  ...
  render() {
    return (
      <div>
        ...
        <div>
          <Search
            fluid {...this.state.input} placeholder={this.state.label} type={this.state.type}
...
export const StepProgress = props => {
  ...
  return (
    <Step.Group ordered>
      <Step completed={props.page > 1}>
        Select Resource
      </Step>
      <Step completed={props.page > 2}>
        Provide Permission Details
      </Step>
      <Step completed={props.page > 3}>
        Review and Submit
      </Step>
    </Step.Group>
  )
